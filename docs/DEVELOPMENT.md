...
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Set up your site schema**        | Run `npm run dev:tina` (or `npx tinacms dev --no-server`) locally. This regenerates `tina/tina-lock.json`. **Commit and push** that file — TinaCloud only reindexes when `tina-lock.json` changes on GitHub. Wait until **Last indexed** updates on [app.tina.io](https://app.tina.io). If indexing is stuck, make a whitespace change to `tina/tina-lock.json`, push again, or use **Reindex** on the branch. |
